...
accountDiv.innerHTML = <div class="account-header"> <p><strong>Hesap Adı:</strong> ${accountData['Hesap Adı']}</p> <p><strong>Hesap Türü:</strong> ${accountData['type']}</p> </div> <div class="account-details"> ${Object.keys(accountData).map(key =>
<p><strong>${key}:</strong> ${accountData[key]}</p>).join('')} </div> ;
